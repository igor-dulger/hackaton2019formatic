...
library AddressEnumerationLib {
    ...
    * //revert
    */
    modifier isNotZero(address _id) {
        require(_id != address(0), "Zero id is forbidden");
        _;
    }
    ...
    * //revert
    */
    modifier shouldExist(ListData storage self, address _id) {
        require(exists(self, _id) || _id==address(0), "this id should exists in list");
        _;
    }
    ...
    * //revert
    */
    modifier shouldNotExist(ListData storage self, address _id) {
        require(!exists(self, _id) && _id!=address(0), "this id should NOT exists in list");
        _;
    }
    ...
    * //revert
    */
    function add(ListData storage self, address _id)
        internal
        isNotZero(_id)
    ...
    * //revert
    */
    function remove(ListData storage self, address _id)
        internal
        isNotZero(_id)
        shouldExist(self, _id)
    {
        ...
        if (lastIndex != deletedIndex) {
            address lastId = self.ids[lastIndex];
    ...
    * //revert
    */
    function getNextId(ListData storage self, address _id)
        internal
        shouldExist(self, _id)
        view
        returns (address)
    {
        ...
        if (index.add(2) > self.ids.length) {
            ...
            return (_id == address(0)) ? self.ids[0] : self.ids[index.add(1)];
        }
    }
    ...
    * @return (boolean key)
    */
    function exists(ListData storage self, address _id)
        internal
        view
        returns(bool)
    {
        if (self.ids.length > 0) {
            return self.ids[self.indexes[_id]] == _id && _id != address(0);
        } else {
            return false;
        }
    }
    ...
    * @return (boolean key)
    */
    function statuses(ListData storage self, address[] memory _ids)
        internal
        view
        returns(bool[] memory)
    {
